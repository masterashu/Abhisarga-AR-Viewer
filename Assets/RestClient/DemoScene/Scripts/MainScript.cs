...
public class MainScript : MonoBehaviour {
	...
	public void Get(){
		...
		RestClient.GetArray<Post>(basePath + "/posts").Then(res => {
			...
			Debug.Log("Users" + JsonHelper.ArrayToJsonString<Post>(res, true) + "Ok");
			...
			Debug.Log("Users" + JsonHelper.ArrayToJsonString<Todo>(res, true) + "Ok");
			...
			Debug.Log("Users" + JsonHelper.ArrayToJsonString<User>(res, true) + "Ok");
			...
			Debug.Log("Header" + requestOptions.GetHeader("Authorization") + "Ok");
			// And later we can clean the default headers for all requests
			RestClient.CleanDefaultHeaders();

		}).Catch(err => /*EditorUtility.DisplayDialog ("Error", err.Message, "Ok") */ Debug.Log(err.Message));
	}
	...
	public void Post(){
		...
		.Then(res => /* EditorUtility.DisplayDialog ("Success", JsonUtility.ToJson(res, true), "Ok") */ Debug.Log("Success " + JsonUtility.ToJson(res, true) + " Ok"))
		.Catch(err => /*EditorUtility.DisplayDialog ("Error", err.Message, "Ok") */ Debug.Log("Error" + err.Message + " Ok"));
	}
	...
	public void Put(){
		...
		RestClient.Put<Post>(currentRequest, (err, res, body) => {
			if(err != null){
				...
				Debug.Log("Error" + err.Message + "Ok");
			}
			else{
				// EditorUtility.DisplayDialog ("Success", JsonUtility.ToJson(body, true), "Ok");
				 Debug.Log("Success" + JsonUtility.ToJson(body, true) + "Ok");
			}
		});
	}
	...
	public void Delete(){
		...
		RestClient.Delete(basePath + "/posts/1", (err, res) => {
			if(err != null){
				...
				Debug.Log("Success" + res.StatusCode.ToString() + "Ok");
